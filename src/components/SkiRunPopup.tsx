...
import loadElevationProfile, { CoordinatesWithElevation, ElevationData, extractPoints } from './ElevationProfileLoader';
...
export class SkiRunPopup extends React.Component<Props, State> {
  constructor(props: Props) {
    ...
    this.state = {feature: null, distance: null, elevationData: null, loadingElevationData: true}
  }
  ...
  componentDidMount() {
    loadRun(this.props.data.lid)
      ...
      .then(
        elevationData => {
          this.setState({elevationData: elevationData, loadingElevationData: false})
        },
        () => {
  ...
  render() {
    ...
    return (
      <Popup.WithTitle title={data.name} badge={Popup.refBadgeFromData(data)}>
        ...
        {
          elevationData && elevationData.ascent > 1 ? <span>Ascent: {Math.round(elevationData.ascent)}m</span> : null
        }
        {
          elevationData && elevationData.descent > 1 ? <span>Descent: {Math.round(elevationData.descent)}m</span> : null
        }
        </div>
        {slopeInfo &&
          <div className={"distance-and-elevation-info"}>
        ...
        {
          (feature !== null && distance !== null && elevationData !== null && this.showHeightProfile())
            ? <HeightProfile
                feature={feature}
                distance={distance}
            ...
            : this.state.loadingElevationData ? <HeightProfilePlaceholder /> : null
        }
      </Popup.WithTitle>
    );
  }
...
function formattedSlope(slopePercent: number) {
  ...
  const degrees = Math.round(Math.abs(Math.atan(slopePercent) / Math.PI * 180)) + "°"
