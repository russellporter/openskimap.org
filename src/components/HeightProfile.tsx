...
import { ElevationData, CoordinatesWithElevation } from './ElevationProfileLoader';
...
> {
  ...
  componentDidMount() {
    import('react-chartjs-2')
      .then(module => this.setState({ LineChart: module.Line }))
  }
  ...
  onHover(chart: Chart, event: MouseEvent, activeElements: Array<{}>): any {
    ...
    let line = turfLineSliceAlong(this.props.feature, position, position, {units: 'meters'})
  ...
  render() {
    ...
    const highlightIndex = this.convertedChartHighlightPosition(this.props.chartHighlightPosition, feature)
    const elevations = elevationData.coordinatesWithElevation.map(coordinate => { return coordinate[2] })
      ...
      datasets: [
        {
          ...
          pointRadius: pointRadiusesWithHighlightAt(highlightIndex, elevations.length),
          pointHitRadius: 10,
          data: elevations
        }
      ]
    ...
    let that = this
    return <div className="height-profile"><LineChart
      data={(element) => data}
      options={{
        legend: {
          display: false
...
function pointRadiusesWithHighlightAt(index: number | null, length: number) {
  const radiuses = Array.apply(null, Array(length)).map(Number.prototype.valueOf, 1);
  if (index !== null) {
    radiuses[index] = 5
  }
  return radiuses
...
function convertChartHighlightPosition(
  ...
  const point = turfNearestPointOnLine(
    feature, chartHighlightPosition, {units: 'meters'},
  )
